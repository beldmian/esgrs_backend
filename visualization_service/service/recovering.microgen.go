...
func (M recoveringMiddleware) GenerateVisualization(ctx context.Context, result types.ESGRatingResult) (vis types.Visualization, err error) {
	defer func() {
		if r := recover(); r != nil {
			M.logger.Log("method", "GenerateVisualization", "message", r)
			err = fmt.Errorf("%v", r)
		}
	}()
